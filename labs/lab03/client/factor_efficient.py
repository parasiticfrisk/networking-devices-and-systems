...
# Assign: Lab 03
# File: factor_efficient.py
#
# Description: Find the prime factors of a large 'semi-prime' number.
#
    ...
    if job.status == dispy.DispyJob.Finished or job.status in (  # most usual case
        ...
        if job.id:  # job may have finished before 'main' assigned id
            ...
            if factor1 != 0:
                found = True
                dispy.logger.info(
                    'job "%i" returned %i * %i = %i, %s jobs pending',
                    job.id,
                    factor1,
                    factor2,
                    factor1 * factor2,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "chunk_scale", type=int, help="chunk size = chunk_scale * log(semi-prime)"
    )
    ...
    if (lower % 2) == 0:
        lower += 1  # make sure we start with an odd number (which could also be prime)
    ...
    cluster = dispy.JobCluster(
        find_factor, nodes=server_nodes, callback=job_callback, loglevel=logging.INFO
    )
    ...
    while (lower <= upper) and (found == False):
        print(
            (
                "Attempting factors in range %i - %i, chunk size %i"
                % (lower, lower + chunk, chunk)
            )
        )
