...
# Assign: Lab 03
# File: primality_efficient.py
#
# Description: Primality test algorithms
#              Generates a list of prime numbers in a user specified range
#              provided at run time. The user can select from naive,
#              Miller-Rabin, or Fermat primality tests. Only the specified test
#              is run.
#
...
def job_callback(job):  # executed at the client
    ...
    if job.status == dispy.DispyJob.Finished or job.status in (  # most usual case
        ...
        if job.id:  # job may have finished before 'main' assigned id
            ...
            if primality == True:
                dispy.logger.info(
                    'job "%i" returned %s, %s jobs pending',
                    job.id,
                    primality,
                    len(pending_jobs),
                )
...
if __name__ == "__main__":
    ...
    print(
        (
            "Finding prime numbers in the range %i - %i on cluster %s"
            % (lower_limit, upper_limit, server_nodes)
        )
    )
    i = lower_limit
